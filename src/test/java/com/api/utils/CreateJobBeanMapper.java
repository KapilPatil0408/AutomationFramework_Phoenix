...
public class CreateJobBeanMapper {
	...
	public static CreateJobPayload mapper(CreateJobBean bean) {
		...
		CustomerProduct customerProduct= new CustomerProduct(bean.getCustomer_product__dop(),
				bean.getCustomer_product__serial_number(), bean.getCustomer_product__imei1(), bean.getCustomer_product__imei2(), bean.getCustomer_product__popurl(), productId, modelId);
		...
		CreateJobPayload payload= new CreateJobPayload(mstServiceLocationId, mstPlatformId, 
				 mstWarrentyStatusId, oemId, customer, customerAddress, customerProduct, problemList);
